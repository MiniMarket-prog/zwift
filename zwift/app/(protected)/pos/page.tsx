...
import { processProductsArray } from "@/utils/product-utils"
...
const POSPage = () => {
  ...
  const fetchData = useCallback(
    async (searchQuery = "") => {
      setIsLoading(true)
      try {
        console.log("🔍 Fetching products in POS page...")
        ...
        const processedProducts = processProductsArray(productsData || [])
        console.log(`📦 Loaded ${processedProducts.length} products in POS page`)

        // Set products state with processed data
  ...
  // In the loadProducts function or wherever products are fetched
  const loadProducts = async (searchQuery = "") => {
    try {
      ...
      // Process the data consistently using our utility function
      const processedProducts = processProductsArray(productsData)
      console.log(`📦 Loaded ${processedProducts.length} products in POS page`)
  ...
  // First, let's fix the SaleItem type definition
  interface SaleItem {
    id: string
    product_id: string
    quantity: number
    price: number
    products?: Product // Change from products: any[] to products?: Product
  }

  // Then update the fetchRecentSales function to use the correct type
  ...
  // Replace the completeSale function with this new implementation
  const completeSale = async (shouldPrint = false) => {
    ...
    try {
      ...
      // Set completed sale data for receipt printing if needed
      if (shouldPrint) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] gap-4 p-4">
      ...
      <SaleConfirmationDialog
        ...
        onCancel={() => setShowSaleDialog(false)}
      />
