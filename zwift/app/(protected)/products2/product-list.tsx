...
export function ProductList({
  ...
  const [totalCount, setTotalCount] = useState(totalProductCount || initialProducts.length)
  ...
  useEffect(() => {
    if (initialProducts.length < 1000) {
      // Only fetch all products if we suspect we're hitting the limit
      refreshProducts()
    }
  }, [])
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    searchTimeout.current = setTimeout(async () => {
      setIsLoading(true)
      try {
        // Use server-side filtering by passing the search term to getAllProducts
        const results = await getAllProducts(value)
  ...
  // Refresh products function with progress indicator
  const refreshProducts = async () => {
    setIsLoading(true)
    const loadingToast = toast({
      title: "Loading products",
      description: "Fetching all products, this may take a moment...",
    })

    try {
      const updatedProducts = await getAllProducts(searchTerm)
      ...
      toast({
        ...
        description: `Successfully loaded ${updatedProducts.length} products`,
      })
    } catch (error) {
  ...
  // Render pagination controls with improved navigation for large datasets
  ...
  const renderListView = () => {
    return (
      <>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between px-4 py-4 border-t gap-4">
          <div className="flex items-center space-x-2">
            <span className="text-sm text-muted-foreground">
              Showing {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, filteredProducts.length)} of{" "}
              {filteredProducts.length} products (Total: {totalCount})
            </span>
            <Select
  ...
  return (
    <div>
      ...
      {/* Edit Product Dialog */}
      {editingProduct && <EditProductDialog product={editingProduct} categories={categories} />}
