...
const AlertsPage = () => {
  ...
  const [aiActiveTab, setAiActiveTab] = useState("restock")
  ...
  // AI Functions
  const getAIInsights = async (action: string) => {
    setIsLoadingAI(true)
    setAiError(null)

    try {
      if (filteredProducts.length === 0) {
        throw new Error("No products available for analysis")
      }
      ...
      if (action === "smart_insights") {
        setSmartInsights(data.result)
      } else {
        setAiInsights(Array.isArray(data.result) ? data.result : [])
      }

      toast({
        title: "✨ AI Analysis Complete",
        description: `Generated ${action.replace("_", " ")} insights successfully`,
      })
    } catch (error: any) {
      console.error("Error getting AI insights:", error)
      const errorMessage = error.message || "Unable to generate insights. Please try again."
  ...
  const applyAIRecommendation = async (insight: AIInsight) => {
    if (!insight.recommendedQuantity) return

    try {
      const { error } = await supabase
        .from("products")
        .update({ stock: insight.recommendedQuantity })
        .eq("id", insight.productId)

      if (error) throw error

      toast({
        title: "✅ Stock Updated",
        description: `${insight.productName} stock updated to ${insight.recommendedQuantity}`,
      })
      ...
      toast({
        title: "Update Failed",
        description: "Unable to apply recommendation. Please try again.",
        variant: "destructive",
      })
    }
  }
          ...
          {showAIPanel && (
            <Card className="border-2 border-blue-200 bg-gradient-to-r from-blue-50 to-purple-50">
              <CardHeader>
                ...
                <CardDescription>Get intelligent recommendations for your inventory management</CardDescription>
              </CardHeader>
              <CardContent>
                {aiError && (
                ...
                <Tabs value={aiActiveTab} onValueChange={setAiActiveTab}>
                  ...
                  <TabsContent value="restock" className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-lg font-semibold">Smart Restocking Suggestions</h3>
                      <Button onClick={() => getAIInsights("restock_suggestions")} disabled={isLoadingAI} size="sm">
                        {isLoadingAI ? (
                            ...
                            <div className="flex items-center justify-between">
                              <div>
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                  ...
                                  {insight.recommendedQuantity && (
                                    <Badge variant="outline">Restock: {insight.recommendedQuantity} units</Badge>
                                  )}
                                </div>
                              </div>
                              {insight.recommendedQuantity && (
                                <Button size="sm" onClick={() => applyAIRecommendation(insight)} className="ml-2">
                                  Apply
                                </Button>
                              )}
                            </div>
                  ...
                  <TabsContent value="pricing" className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-lg font-semibold">Price Optimization</h3>
                      <Button onClick={() => getAIInsights("price_optimization")} disabled={isLoadingAI} size="sm">
                        {isLoadingAI ? (
                          <Loader2 className="h-4 w-4 animate-spin mr-2" />
                        ) : (
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              {insight.suggestedPrice && (
                                <Badge variant="outline">
                                  Suggested: {formatCurrency(insight.suggestedPrice, currentCurrency, language)}
                                </Badge>
                              )}
                            </div>
                          </div>
                  ...
                  <TabsContent value="forecast" className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <Button onClick={() => getAIInsights("demand_forecast")} disabled={isLoadingAI} size="sm">
                        {isLoadingAI ? (
                          <Loader2 className="h-4 w-4 animate-spin mr-2" />
                        ) : (
                            ...
                            <div className="grid grid-cols-2 gap-4 mt-2">
                              {insight.demand7Days && (
                                <div>
                                  ...
                                  <p className="font-semibold">{insight.demand7Days} units</p>
                                </div>
                              )}
                              {insight.demand30Days && (
                                <div>
                                  <p className="text-xs text-muted-foreground">30-Day Forecast</p>
                                  <p className="font-semibold">{insight.demand30Days} units</p>
                                </div>
                              )}
                            </div>
                            {insight.trend && (
                  ...
                  <TabsContent value="insights" className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <Button onClick={() => getAIInsights("smart_insights")} disabled={isLoadingAI} size="sm">
                        {isLoadingAI ? (
                          <Loader2 className="h-4 w-4 animate-spin mr-2" />
                        ) : (
                    ...
                    {smartInsights && (
                      <div className="space-y-4">
                        {smartInsights.criticalInsights.length > 0 && (
                          <div className="p-3 bg-red-50 rounded-lg border border-red-200">
                            <h4 className="font-semibold text-red-800 mb-2">Critical Insights</h4>
                            <ul className="space-y-1">
                              {smartInsights.criticalInsights.map((insight, index) => (
                        ...
                        {smartInsights.immediateActions.length > 0 && (
                          <div className="p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                            <h4 className="font-semibold text-yellow-800 mb-2">Immediate Actions</h4>
                            <ul className="space-y-1">
                              {smartInsights.immediateActions.map((action, index) => (
                        ...
                        {smartInsights.longTermStrategy.length > 0 && (
                          <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                            <h4 className="font-semibold text-green-800 mb-2">Long-term Strategy</h4>
                            <ul className="space-y-1">
                              {smartInsights.longTermStrategy.map((strategy, index) => (
