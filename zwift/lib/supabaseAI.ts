...
// If you need to bypass RLS for server-side AI functions,
// you would typically use a service role key here:
// const supabaseServiceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY!;

// Create a Supabase client (using anon key as per your provided file)
export const createClient = () => {
  return createSupabaseClient(supabaseUrl, supabaseKey)
}

// Define types (keeping your existing ones and adding more specific ones for AI tools)
...
// Comprehensive Product type (from your existing file, ensuring all fields are optional if not always selected)
export type Product = {
  id: string
  name: string
  price: number
...
type ProductForSlowMovingQuery = {
  id: string
  name: string
  stock: number
  min_stock: number
...
type SaleItemForSlowMovingQuery = {
  product_id: string
  quantity: number
}

// --- END NEW/UPDATED TYPES ---
...
export async function updateCategoryName(categoryId: string, name: string) {
  try {
    ...
    return data as Category // Cast to Category type
  } catch (error) {
...
export async function addProduct(productData: {
  ...
  try {
    ...
    return data as Product // Cast to Product type
  } catch (error) {
...
export async function getProductPack(productId: string) {
  ...
  try {
    ...
    return data as Product | null // Cast to Product type
  } catch (error) {
...
export async function getProductUnit(packId: string) {
  ...
  try {
    ...
    return unit as Product // Cast to Product type
  } catch (error) {
...
export async function getLowStockProducts() {
  ...
  try {
    ...
    return lowStockProducts as Product[] // Cast to Product type
  } catch (error) {
...
export async function getExpiringProducts(daysThreshold = 30) {
  ...
  try {
    ...
    return expiringProducts as Product[] // Cast to Product type
  } catch (error) {
...
export async function getPurchaseOrderById(id: string) {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .single()
    if (error) throw error
    return { data: data as PurchaseOrder & { purchase_order_items: PurchaseOrderItem[] }, error: null }
  } catch (error) {
...
export async function createPurchaseOrder(orderData: {
  ...
  try {
    ...
    return {
      data: {
        ...(orderDataResult as PurchaseOrder), // Cast to PurchaseOrder
        items: orderItems as PurchaseOrderItem[], // Cast to PurchaseOrderItem array
      },
      error: null,
    }
  } catch (error) {
...
async function updateStockFromPurchaseOrder(purchaseOrderId: string) {
  ...
  try {
    ...
    for (const item of orderItems as PurchaseOrderItem[]) {
...
export async function generatePurchaseOrder(productIds: string[]) {
  ...
  try {
    ...
    return {
      ...
      products: products as Product[], // Cast to Product array
  ...
  const slowMovingProducts = (allProducts || []) // Ensure allProducts is an array
    ...
    .map((product: ProductForSlowMovingQuery) => {
      ...
      return {
        ...
        avg_daily_velocity: Number.parseFloat(((productSales[product.id] || 0) / periodDays).toFixed(2)),
      }
    })
