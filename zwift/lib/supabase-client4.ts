...
export async function getLowStockProducts(): Promise<Array<Product & { min_stock: number }>> {
  const supabase = createClient()

  try {
    // Get products with zero stock but only if min_stock is at least 1
    const { data: zeroStockProducts, error: zeroStockError } = await supabase
      .from("products")
      .select("*")
      .eq("stock", 0)
    ...
    if (zeroStockError) {
      console.error("Error fetching zero stock products:", zeroStockError)
